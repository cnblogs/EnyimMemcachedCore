...
namespace Enyim
{
    ...
    internal class HashkitCrc32 : HashAlgorithm, IUIntHashAlgorithm
    {
        ...
        private void FinalizeHash()
        {
            ...
            _currentHash = ((~_currentHash) >> 16) & 0x7fff;
            // _currentHash = (~_currentHash);
        }
        ...
        protected override byte[] HashFinal()
        {
            ...
            return BitConverter.GetBytes(_currentHash);
        }

        public uint CurrentHash { get { return _currentHash; } }
        ...
        private unsafe void UnsafeHashCore(byte[] buffer, int offset, int count)
        {
            fixed (byte* ptr = buffer)
            {
                ...
                while (count > 0)
                {
                    _currentHash = CrcTable[(byte)(_currentHash ^ (*current))] ^ (_currentHash >> 8);
