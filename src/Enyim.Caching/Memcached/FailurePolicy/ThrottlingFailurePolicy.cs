...
namespace Enyim.Caching.Memcached
{
    ...
    public class ThrottlingFailurePolicy : INodeFailurePolicy
    {
        ...
        bool INodeFailurePolicy.ShouldFail()
        {
            ...
            {
                if (_logIsDebugEnabled) _log.Debug("Setting fail counter to 1.");

                _failCounter = 1;
            }
            else
            {
                var diff = (int)(now - _lastFailed).TotalMilliseconds;
                if (_logIsDebugEnabled) _log.DebugFormat("Last fail was {0} msec ago with counter {1}.", diff, _failCounter);
            ...
            {
                if (_logIsDebugEnabled) _log.DebugFormat("Threshold reached, node will fail.");
            ...
            if (_logIsDebugEnabled) _log.DebugFormat("Current counter is {0}, threshold not reached.", _failCounter);
