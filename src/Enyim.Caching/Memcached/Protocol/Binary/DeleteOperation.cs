        ...
        protected override IOperationResult ProcessResponse(BinaryResponse response)
        {
			...
			if (log.IsDebugEnabled)
				if (response.StatusCode == 0)
					log.DebugFormat("Delete succeeded for key '{0}'.", Key);
				else
					log.DebugFormat("Delete failed for key '{0}'. Reason: {1}", Key, Encoding.ASCII.GetString(response.Data.Array, response.Data.Offset, response.Data.Count));
#endif
            if (response.StatusCode == 0)
            {
                return result.Pass();
